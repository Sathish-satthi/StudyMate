  ...
  return (
    <div className="container fade-in" style={{ padding: '60px 20px' }}>
      <header style={{ marginBottom: '48px' }}>
        <h1 style={{ fontSize: '2.5rem', marginBottom: '8px' }}>Important Questions 💡</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Select a subject to view or contribute critical exam questions.</p>
      </header>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '24px' }}>
        {subjects.map((sub) => (
          <Link key={sub.id} to={`/questions/${sub.id}`} style={{ textDecoration: 'none' }}>
            <div style={{ 
              background: 'white', 
              padding: '24px', 
            ...
            onMouseOut={(e) => {
              ...
              <div style={{ marginTop: '16px', color: 'var(--secondary-color)', fontSize: '0.75rem', fontWeight: '700', textTransform: 'uppercase' }}>
                View Exam Questions →
              </div>
            </div>
          </Link>
